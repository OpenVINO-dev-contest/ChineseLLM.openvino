...
# input & output names
past_names = [f"past_key_values.{i}.{name}" for i in range(28) for name in ["key", "value"]]
present_names = [f"present_key_values.{i}.{name}" for i in range(28) for name in ["key", "value"]]
output_names = ["logits"] + present_names
default_past_key_values = {
    k: np.zeros((1, 1, 2, 128), dtype=np.float32) for k in past_names
}
...
class ChatGLMModel():
    ...
    def __init__(self, tokenizer_path, onnx_model_path='./onnx_model/chatglm.onnx') -> None:
        ...
        self.request = core.compile_model(model=model, device_name=args.device).create_infer_request()
    ...
    def generate_iterate(self, prompt: str, max_generated_tokens, top_k=20, top_p=0.7, temperature=1):
        ...
        while True:
            ...
            past_key_values = tuple(self.request.get_tensor(key).data for key in present_names)
            ...
            if next_token == self.eos_token_id or len(output_tokens) > max_generated_tokens:
                break
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--max_sequence_length', default=32, required=False, type=int,
                        help='Required. maximun lengh of output')
    parser.add_argument('-d', '--device', default='CPU', required=False, type=str,
    ...
    for text in ov_chatglm.generate_iterate(args.prompt, args.max_sequence_length):
        print(text)